...
Proof.
  intros. change (S (f p)) with ((fun p0 => S (f p0)) p). apply H1.
...
Lemma ic_cont_closed: forall (D C : cpoType) (f : D =-> C) (P : prdc D) (S : prdc C),
    (forall p, P p -> S (f p)) ->
    forall p, ideal_closure P p -> ideal_closure S (f p).
Proof.
  intros D C f P S H p icH.
  apply cont_closed with (P:=P).
  apply ideal_closure_closed.
  intros p0 H0. apply ideal_closure_sub. apply H. auto. auto.
Qed.
...
Lemma ic_cont2_closed: forall (D1 D2 D3 : cpoType)
                        (f : D1 =-> (D2 -=> D3))
                        (P: prdc D1) (Q: prdc D2) (S: prdc D3),
    (forall p q, P p -> Q q -> S (f p q)) ->
    forall p q, ideal_closure P p -> ideal_closure Q q -> ideal_closure S (f p q).
Proof.
  intros D1 D2 D3 f P Q S H p q icHP icHQ.
  apply cont2_closed with (P:=P) (Q:=Q).
  apply ideal_closure_closed.
  intros p0 q0 H0 H1. 
  apply ideal_closure_sub. apply H; auto. auto. auto.
Qed.
