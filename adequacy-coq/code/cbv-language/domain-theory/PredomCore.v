...
Lemma cppoExpAxiom : @CatExp.axiom cppoProdCatType
                                   exp_cppoType
                                   (fun D0 D1 => @EV D0 D1)
                                   (fun D0 D1 D2 => @CURRY D0 D1 D2).
move => D0 D1 D2 h.
...
Lemma prodi_cpo_axiom : CPO.axiom (fun (f : natO =-> prodI_ordType D) (i:I) =>
                                     lub (Proj D i << f)).
...
Canonical Structure prodi_cpoMixin :=
  @CpoMixin (prodI_ordType D) _ prodi_cpo_axiom.
Canonical Structure prodi_cpoType :=
  Eval hnf in @CpoType (prodI_ordType D) prodi_cpoMixin.
...
Lemma cpoProdIAxiom : forall I:Type, @CatIProduct.axiom _ I
                                                   (@prodi_cpoType I)
                                                   (@PROJ I) (@PRODI_fun I).
move => I A X f. split.
...
Variable I : Type.
Variable D : I -> cppoType.

Lemma prodi_cppo_axiom : @Pointed.axiom (prodi_cpoType D) (fun (i:I) => PBot).
Proof.
  unfold Pointed.axiom. intros x a. apply leastP.
Qed.


Canonical Structure prodi_cppoMixin := PointedMixin prodi_cppo_axiom.
Canonical Structure prodi_cppoType  := Eval hnf in CppoType prodi_cppoMixin.

Lemma Proj_cont_cppo i : continuous (@Proj I D i:ordCatType prodi_cppoType _).
move => i c. simpl. by apply: lub_le_compat.
Qed.

Definition PROJ_cppo i := Eval hnf in mk_fcont (Proj_cont_cppo i).

Lemma prod_fun_cont_cppo (D1:cpoType) (f:forall i:I, D1 =-> D i) :
   continuous (@Prodi_fun I _ D1 (fun i => (f i)) : ordCatType D1 prodi_cppoType).
move => D1 f c i.
simpl. rewrite (fcontinuous (f i)). by apply: lub_le_compat => n.
Qed.

Definition PRODI_fun_cppo D1 (f:forall i:I, D1 =-> D i) : D1 =-> prodi_cppoType :=
  Eval hnf in mk_fcont (prod_fun_cont_cppo f).
